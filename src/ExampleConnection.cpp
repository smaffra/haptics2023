...
static void handleDeviceEvent(const LEAP_DEVICE_EVENT *device_event){
  ...
  if(result == eLeapRS_InsufficientBuffer){
    ...
    deviceProperties.serial = static_cast<char*>(realloc(deviceProperties.serial, deviceProperties.serial_length));
    result = LeapGetDeviceInfo(deviceHandle, &deviceProperties);
    if(result != eLeapRS_Success){
...
void setFrame(const LEAP_TRACKING_EVENT *frame){
  LockMutex(&dataLock);
  if(!lastFrame) lastFrame = static_cast<LEAP_TRACKING_EVENT*>(malloc(sizeof(*frame)));
  *lastFrame = *frame;
